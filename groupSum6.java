 ...
 */
 ...
 public boolean groupSum6(int start, int[] nums, int target) {
  ...
  // recurse as if nums[start] was chosen
  chosen = groupSum6(start + 1, nums, target - start);
  
  if(chosen)return true;
  
  // recusrse as if nums[start] was not chosen
  chosen = groupSum6(start + 1, nums, target);
